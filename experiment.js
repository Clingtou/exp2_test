/*
  Experiment 2: Ultimatum game receiver proposal ratings with font-size amount displays.
  Replace PROLIFIC_COMPLETION_CODE before launching on Prolific.
*/

const DATAPIPE_EXPERIMENT_ID = "DhHz3SKPYj53";
const PROLIFIC_COMPLETION_CODE = "CT5YG8HI";
const BASE_PAYMENT_USD = 1.00;
const BONUS_DRAW_PERCENT = 10;
const FONT_MANIPULATION_RATIO = 2;
...
const data_filename = `${subject_id}_${session_id}_${Date.now()}_ultimatum_exp2.csv`;
const preview_mode = jsPsych.data.getURLVariable("preview") === "1" || prolific_pid === "missing";
const studyLockKey = `ultimatum_exp2_status_${prolific_pid}_${study_id}`;
...
const areaConditions = [
  { area_condition: "equal_size", you_font_multiplier: 1, other_font_multiplier: 1 },
  { area_condition: "you_larger", you_font_multiplier: FONT_MANIPULATION_RATIO, other_font_multiplier: 1 },
  { area_condition: "other_larger", you_font_multiplier: 1, other_font_multiplier: FONT_MANIPULATION_RATIO }
];
...
function amountTextBlockHtml(x, y, label, amount, multiplier, compact = false) {
  const personFontSize = Math.round((compact ? 29 : 34) * multiplier);
  const amountFontSize = Math.round((compact ? 25 : 30) * multiplier);
  const lineGap = Math.round((compact ? 40 : 48) * multiplier);
  return `
    <g class="amount-text-block" transform="translate(${x} ${y})">
      <text class="amount-person" x="0" y="0" text-anchor="middle" style="font-size: ${personFontSize}px;">${label}</text>
      <text class="amount-cents" x="0" y="${lineGap}" text-anchor="middle" style="font-size: ${amountFontSize}px;">${amount} cents</text>
    </g>
  `;
}

function amountDisplayHtml(condition, options = {}) {
  const compact = options.compact === true;
  const viewBoxHeight = compact ? 500 : 620;
  const cx = 390;
  const cy = viewBoxHeight / 2;
  const horizontalOffset = compact ? 170 : 210;
  const youPosition = condition.position_condition === "left"
    ? { x: cx - horizontalOffset, y: cy - 24 }
    : { x: cx + horizontalOffset, y: cy - 24 };
  const otherPosition = condition.position_condition === "left"
    ? { x: cx + horizontalOffset, y: cy - 24 }
    : { x: cx - horizontalOffset, y: cy - 24 };

  return `
    <svg class="amount-display-svg" viewBox="0 0 780 ${viewBoxHeight}" role="img" aria-label="Text display showing the proposed allocation">
      ${amountTextBlockHtml(youPosition.x, youPosition.y, "you", condition.you, condition.you_font_multiplier, compact)}
      ${amountTextBlockHtml(otherPosition.x, otherPosition.y, "other", condition.other, condition.other_font_multiplier, compact)}
    </svg>
  `;
}

function exampleAmountDisplayHtml(condition) {
  return amountDisplayHtml({
    you: 50,
    other: 50,
    you_font_multiplier: 1,
    other_font_multiplier: 1,
  ...
  const questions = [
    ...
    },
    {
      ...
      exampleHtml: `
          ...
          <div class="example-display">${exampleAmountDisplayHtml(conditionInfo)}</div>
        </div>
      `,
      options: [
  ...
  return {
    ...
    stimulus: `
      <div class="stage-message">
        ...
        ${isChoice
          ? `<p>You will now make <strong>${exp2Splits.length}</strong> choices.</p>
...
function proposalDecisionTrial(condition, split, trialIndex) {
  ...
  const html = shellHtml(`
    <div class="stimulus-content exp2-decision-content">
      ...
      <div class="amount-display-wrap exp2-decision-display-wrap">${amountDisplayHtml(stimulusCondition)}</div>
  ...
  return {
    ...
    data: {
      ...
      exp2_trial_number: trialNumber,
      split_id: split.split_id,
      you_cents: split.you,
      other_cents: split.other,
      amount_difference_cents: split.other - split.you,
      area_condition: condition.area_condition,
      you_font_multiplier: condition.you_font_multiplier,
      other_font_multiplier: condition.other_font_multiplier,
      position_condition: condition.position_condition,
...
function exp2TaskTrials(conditionInfo) {
  const randomizedSplits = jsPsych.randomization.shuffle(exp2Splits);
...
async function buildAndRunExperiment() {
  ...
  timeline.push({
    type: jsPsychPreload,
    images: ["ModifiedMullerLyer.png", "instruction-flow.png"],
  ...
  timeline.push({
    timeline: [...exp2TaskTrials(conditionInfo), ...postQuestionnaireTrials()],
    conditional_function: function () {
